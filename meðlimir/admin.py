...
from meðlimir.models import Medlimur

# Dynamically build TabularInlines for any ManyToMany on Uppskrift that points to meðlimir.
...
for f in Uppskrift._meta.get_fields():
    ...
    if getattr(f, "many_to_many", False) and getattr(f, "remote_field", None):
        ...
        # remote may be the class object; check identity with meðlimir
        if remote is Medlimur:
            through_model = f.remote_field.through
            _inlines.append(_make_through_inline(through_model, f.name))

class meðlimirAdmin(admin.ModelAdmin):
...
admin.site.register(Medlimur, meðlimirAdmin)
